...
public class TestTimeFilling extends TestCase {
    ...
    public void checkAddHoursWorkCorrectly() {
        KathyLog.info("Clicking on the first activity");
        ...
        KathyLog.info("Filling cells");
        quantity.fill("1");

        KathyLog.debug("Checking how the cells are filled with hours and the activity field - with the name of the actvity");
    ...
    public void checkTotalTimeIsChanged() {
        ...
        KathyLog.info("Logging hours for today");
        tableJournalService.logHoursForToday("1");

        KathyLog.info("Saving hours");
        ...
        KathyLog.debug("Checking that the total reported time has been changed after logging some new hours");

        Assert.assertNotSame(totalTimeBeforeChanges, totalTimeAfterChanges, "Verifying time before and after change");
    }
    ...
    public void checkDataNotSavedAfterRefreshingPage() {
        ...
        KathyLog.debug("Checking data is not saved after refreshing the page");
    ...
    public void checkDataNotSavedAfterClickingCancel() {
        ...
        KathyLog.info("Clicking Cancel button");
        controlPanelService.cancelButtonClick();

        KathyLog.debug("Checking data is not saved after clicking Cancel button");
    ...
    public void checkTwoEqualActivitiesCannotBeAdded() {
        ...
        KathyLog.info("Sending the same keys to the newly created activity");
        fillingActivityService.fill(ActivityNames.COMMUNICATION.getValue());

        KathyLog.info("Clicking Save button");
        controlPanelService.clickSaveHoursButton();

        KathyLog.info("Closing warning message");
        warningMessagesService.clickToCloseWarningMesssage();

        KathyLog.info("Clicking Save button");
        controlPanelService.clickSaveHoursButton();

        KathyLog.debug("Checking that the hours can't be added to the activity with the same name");
    ...
    public void checkItIsForbiddenToAddMoreThanTwentyFourHoursPerDay(){
        KathyLog.info("Clicking to add Activity");
        tableJournalService.clickActivityButtonWithIndex(0);

        KathyLog.info("Filling Activity cell");
        fillingActivityService.fill(ActivityNames.COMMUNICATION.getValue());


        KathyLog.info("Filing all the working day cells with 25 hours");
        quantity.fill("25");
        Sleeper.sleep(1);

        KathyLog.info("Clicking to save hours");
        controlPanelService.clickSaveHoursButton();

        KathyLog.debug("Checking Warning message is displayed when User attempts to save more than 24 hours");
        Assert.assertTrue(warningMessagesService.checkModalWindowIsDisplayed(), "Warning message is not displayed, User can save more than 24 hours a day");
    }
}
